...
export const errorMap: ZodErrorMap = (baseError, ctx) => {
  ...
  if (baseError.code === 'invalid_union') {
      ...
      if (unionError.code === 'invalid_type' || unionError.code === 'invalid_literal') {
        ...
        if (typeOrLiteralErrByPath.has(flattenedErrorPath)) {
          ...
          typeOrLiteralErrByPath.set(flattenedErrorPath, {
            ...
            received: (unionError as any).received, // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    const messages: string[] = [
      prefix(baseErrorPath, typeOrLiteralErrByPath.size ? 'Did not match union:' : 'Did not match union.'),
    ];
    return {
      message: messages
        .concat(
  ...
  if (baseError.code === 'invalid_literal' || baseError.code === 'invalid_type') {
    return {
      message: prefix(
        ...
        getTypeOrLiteralMsg({
          code: baseError.code,
          received: (baseError as any).received, // eslint-disable-line @typescript-eslint/no-explicit-any
