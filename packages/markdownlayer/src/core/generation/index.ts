...
async function generateInner(options: GenerateInnerOptions) {
  ...
  await writeRootIndexFiles({ mode, outputFolder, configPath, generations });
...
async function generateDocuments(options: GenerateDocsOptions): Promise<GenerationResult> {
  ...
  for (const file of files) {
    ...
    // write json file
    const outputFilePath = join(outputFolder, type, `${idToFileName(document._id)}.json`);
    await writeFile(outputFilePath, JSON.stringify(document, null, 2), { encoding: 'utf8' });
  ...
  if (collectionChanged || docs.length == 0) {
    ...
    await writeFile(outputFilePath, JSON.stringify(docs, null, 2), { encoding: 'utf8' });
    ...
    const lines: string[] = [
      ...
      '',
      ...docs.map(
        (doc) => `import ${makeVariableName(doc._id)} from './${idToFileName(doc._id)}.json'${assertStatement};`,
      ),
      '',
...
async function writeRootIndexFiles({ mode, outputFolder, configPath, generations }: WriteRootIndexFilesOptions) {
