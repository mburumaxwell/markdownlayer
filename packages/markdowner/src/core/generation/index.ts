...
import type { DocumentDefinition, DocumentMeta, MarkdownerConfig, MarkdownerConfigPlugins } from '../types';
...
export async function generate(options: GenerateOptions) {
  ...
  let outputFolder = path.join(cwd, '.markdowner');
...
async function generateInner(options: GenerateInnerOptions) {
  ...
  console.log(`Generated ${total} documents (${cached} from cache) in .markdowner`);
}
...
async function writePackageJson({ outputFolder, configHash }: WritePackageJsonFileOptions) {
  const packageJson: PackageJson & { typesVersions: any } = {
    name: 'dot-markdowner',
    description: 'This package is auto-generated by markdowner.',
...
async function writeTypesFile({ outputFolder, documentTypeNames }: WriteIndexFileOptions) {
  ...
  let lines: string[] = [autogeneratedNote, ''];
  lines.push(...[`import type { ImageData, BaseDoc } from 'markdowner/core';`, '']);
